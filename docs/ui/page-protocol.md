...
- 桌面端页面内容使用可伸缩全宽工作区；主数据表与表单保持紧凑密度，不使用大幅营销式标题或装饰卡片。
- 页面不设置独立的页首标题、说明或指标摘要；当前导航高亮和浏览器标题承担页面身份识别，页面内容从首个检索、操作、表单或内容区直接开始。
- 面板、表格、弹窗和输入框使用小圆角、细边界与克制阴影；状态不能只依赖颜色表达。
- 共享视觉规则由 `static/components/precision.css` 承担，业务页面不得复制导航、按钮、输入、表格或弹窗皮肤。
- 窄屏允许导航横向滚动和表格框内滚动，禁止页面根节点出现非预期横向溢出；主要操作与当前用户入口仍须可达。
...
- 跨页面类名按职责命名，例如 `data-section`、`search-command`、`choice-card`；页面类名使用清晰业务前缀。废弃的 `workspace-header`、`search-hero`、`materials-*` 共享类、`inquiry-*` 搜索壳和 `product-modal` 等旧类名由检查器阻止恢复。
