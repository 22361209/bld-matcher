...
## Precision Workbench Visual System

当前 UI 协议版本使用“精密工业工作台”视觉语言：深石墨导航承担全局定位，冷灰网格画布提供工业背景，白色表面承载可操作内容，BLD 蓝只用于主操作和焦点，信号橙只用于系统边界与高注意状态。

- 桌面端页面内容使用可伸缩全宽工作区；主数据表与表单保持紧凑密度，不使用大幅营销式标题或装饰卡片。询价首页和生产料单首页是稳定搜索入口，可保留居中的大标题、说明、关键统计与垂直留白。
- 除两个首页外，页面不设置独立的页首标题、说明或指标摘要；当前导航高亮和浏览器标题承担页面身份识别，页面内容从首个检索、操作、表单或内容区直接开始。
- 面板、表格、弹窗和输入框使用小圆角、细边界与克制阴影；状态不能只依赖颜色表达。
- 共享视觉规则由 `static/components/precision.css` 承担，业务页面不得复制导航、按钮、输入、表格或弹窗皮肤。
- 窄屏允许导航横向滚动和表格框内滚动，禁止页面根节点出现非预期横向溢出；主要操作与当前用户入口仍须可达。位于横向滚动导航内的下拉菜单必须以相对触发按钮的视口定位呈现，不得被滚动容器裁切。
...
- 跨页面类名按职责命名，例如 `data-section`、`search-command`、`choice-card`；页面类名使用清晰业务前缀。`workspace-header` 已废弃并由检查器阻止恢复；`search-hero` 仅允许询价首页作为兼容类名使用，生产料单首页使用页面级 `material-landing`。不得新增其他 `search-hero`、`materials-*` 共享类、`inquiry-*` 搜索壳或 `product-modal` 等旧类名。
