...
- Web、API 和 CLI 是输入适配器，只调用 Service。
- Service 可以调用 Domain 和 Ports，负责一次用例的事务边界。
- Domain 不依赖 Flask、SQLite、Path 或供应商 SDK。
- Repository 和文件存储是基础设施实现。
- 一个模块不得导入另一个模块的路由、模板或数据库实现。

## Current Debt And Ratchet

产品、询价、报价、客户、客户资料、合同、材料、后台管理、登录和产品同步都已迁入领域 Service/Repository 边界。产品与询价页面入口已按目录、价格、记录、媒体、匹配、下载和映射职责拆分；询价 Excel、合同文档与材料持久化分别按读写、解析和渲染职责拆分，旧入口只保留兼容门面。`app/database.py` 只保留 Schema、`connect()` 和迁移调用；项目检查器禁止业务函数回流。路由数据库导入/SQL、跨路由导入、daemon 线程和异常文本外泄的遗留计数均为零。

全部完整页面继承 `templates/base.html` 并声明唯一 page ID 和 page type；独立页面、内联脚本/事件和内联样式白名单均为空。精密工业工作台视觉由共享 `precision.css` 覆盖导航、页面标题、表单、表格、面板和窄屏层级，页面层只负责业务布局。CSS 已按基础层、共享组件层和页面层拆分，基础与共享层禁止业务选择器，页面样式必须由模板认领，并受 1400/1000/600 行硬上限、零 ID 选择器和 `!important` 冻结门禁约束。`/api/internal/*` 仍是明确登记的兼容 API，只能复用现有 Service，不能扩展新资源；旧 `/api/quotes` 已由 ADR 0013 删除，报价对外只保留 `/api/v1/quotes`。
...
5. 已完成：`app/modules/contracts/`、`materials/` 和 `admin/` 负责事务、审计与文件补偿；全部页面纳入基础模板协议，模板内联资源清零。
6. 已完成：readiness/业务探针、结构化日志、保留期执行器和历史数据库 fixture 矩阵；剩余运行/路由债务清零。
...
- 容器先运行 `scripts/init_database.py`，成功后才启动 Gunicorn；Web 请求不创建后台线程。
- `/health/ready` 通过只读连接检查数据库、迁移和最小业务条件，不执行初始化；`scripts/runtime_probe.py` 追加登录页业务探针。
- `scripts/cleanup_runtime.py` 默认 dry-run；显式应用后清理过期运行数据并写审计，活跃 artifact 文件受保护。
...
- 页面合同检查阻断独立 HTML、无效 page type、内联代码和重复 page ID；CSS 门禁同时阻断跨层业务选择器、孤立页面资产、ID 选择器、未批准 `!important` 和单文件重新聚集；页面脚本按 `data-page` 独立初始化。
- `scripts/openapi_snapshot.py --check` 阻断未审查的 API 路径、Schema、Scope、参数和响应漂移。
- Ruff 当前阻断语法错误、未使用导入和未使用变量；Pyright 阻断共享平台、合同、材料、询价 Excel、后台管理、产品同步与运行脚本的类型漂移。其余历史领域模块的全量类型基线尚未清零，后续只能扩大门禁范围，不能移除现有范围。
- 已拆处理代码执行双重容量门禁：兼容门面最多 80 行，职责模块最多 360 行；完成拆分的文件必须从遗留行数白名单删除，不能以新白名单恢复聚集。
