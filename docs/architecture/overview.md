...
产品、询价、报价、合同、材料、发货、后台管理、登录、产品同步和货物识别都已迁入领域 Service/Repository 边界。产品与询价页面入口已按目录、价格、记录、媒体、匹配、下载和映射职责拆分；询价 Excel 与合同文档分别按读写、解析和渲染职责拆分，旧入口只保留兼容门面。`app/database.py` 只保留 Schema、`connect()` 和迁移调用；项目检查器禁止业务函数回流。路由数据库导入/SQL、跨路由导入、daemon 线程和异常文本外泄的遗留计数均为零。
...
- Ruff 当前阻断语法错误、未使用导入和未使用变量；Pyright 阻断共享平台、合同、询价 Excel、后台管理、产品同步、货物识别与运行脚本的类型漂移。其余历史领域模块的全量类型基线尚未清零，后续只能扩大门禁范围，不能移除现有范围。
