...
| --- | --- | --- | --- |
| `ARCH-001` | 部分自动 | 全部现有业务、登录与同步均有 Service/Repository 边界，行为测试覆盖事务与补偿 | 新领域继续复用模块合同测试 |
...
| `DATA-004` | 部分自动 | 产品与业务数据同步媒体、材料 Excel、合同 PDF 和物料图纸写入具备原子替换或失败补偿测试 | 抽取正式 FileStore 端口并扩大覆盖 |
...
| `SEC-004` | 自动 | 外部 AI 供应商调用层已删除，请求参数不存在决定外发地址、密钥、代理或目标的入口 | 重新引入外部 AI 供应商前必须先写 ADR 与 egress 测试 |
...
| `API-004` | 人工/待建 | 后台任务平台与统一任务接口已随 ADR 0023 删除，当前无长任务用例 | 新增长任务时按宪章重建统一任务接口、幂等取消与合同测试 |
| `API-005` | 自动 | v1 只返回 Principal 所有的限时 artifact；响应无绝对路径；跨 Key、过期、下载和保留期清理测试 | 无 |
| `API-006` | 自动 | 产品/询价 Web、旧 API 和 v1 共用服务；消费者合同验证同一结果且 artifact 禁止越权 | 扩大到合同消费者 |
| `API-007` | 自动 | 旧 API 精确白名单；适配器迁移不增端点；报价和询价新旧合同测试；OpenAPI 快照阻断漂移 | 消费者迁移后删除兼容端点 |
| `OPS-001` | 自动 | daemon 计数为零；Web 请求线程不创建后台任务 | 禁止新增进程内后台线程 |
| `OPS-002` | 人工/待建 | 外部 AI 供应商调用层已删除，当前无外部调用方 | 新增外部调用时必须先落地超时、有限重试、并发限制和耗时/供应商/费用指标 |
| `OPS-003` | 自动 | Web 与 API 使用结构化日志字段与稳定错误；异常文本外泄计数为零 | 将日志 Schema 接入集中采集验证 |
| `OPS-004` | 自动 | 无副作用 Web readiness 分项检查 DB/迁移/业务探针，`runtime_probe` 校验 live/ready/login | 部署平台把 probe 设为发布阻断 |
| `OPS-005` | 部分自动 | 可配置保留期、默认 dry-run、受控路径、活跃 artifact 保护、应用清理审计与测试 | 在生产调度 maintenance profile 并监控执行结果 |
