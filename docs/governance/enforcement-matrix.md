...
| `SEC-005` | 自动 | Key 单次显示、`no-store`、哈希、到期、明文字段删除和可配置轮换提醒均有测试；提醒不自动删除 | 无 |
...
| `GOV-003` | 自动 | 本机和 CI 共用 `scripts/verify.py`，阻断平台、合同、材料、询价 Excel 与运行边界类型漂移，并从隔离数据库精确比较 Flask 路由与 OpenAPI 提交快照 | 逐步把其余历史领域模块纳入 Pyright |
