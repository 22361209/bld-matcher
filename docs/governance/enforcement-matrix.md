...
| `ARCH-006` | 自动 | 白名单相对 base ref 不得新增或增加；路由 DB、SQL、daemon 和异常外泄计数已清零，只保留精确兼容 API | 消费者迁移后删除兼容 API |
...
| `GOV-003` | 自动 | 本机和 CI 共用 `scripts/verify.py`，阻断平台/运行边界类型漂移，并从隔离数据库精确比较 OpenAPI 提交快照 | 逐步把历史 Excel 与领域模块纳入 Pyright |
