...
| --- | --- | --- | --- |
| `ARCH-001` | 部分自动 | 报价已完成 Domain/Service/Repository/Web/API 纵向切片；其余路由依赖和 SQL 数量棘轮 | 产品与询价复用同一模式 |
| `ARCH-002` | 部分自动 | 模块层依赖检查阻断 Domain/Service/Adapter 反向依赖；ADR 评审 | 增加跨领域公开服务依赖图 |
| `ARCH-003` | 自动 | 新路由数据库导入失败；报价路由已清零；旧 SQL 调用按文件计数只减不增 | 逐域把计数降为零 |
...
| `DATA-005` | 部分自动 | 容器预初始化、跨进程迁移、迁移记录和历史报价表升级 fixture | 阶段 6 增加完整历史数据库 fixture 矩阵 |
...
| `SEC-003` | 部分自动 | 报价 Service 同一事务完成校验、版本写入、before/after 和 Principal 审计；平台幂等记录 request ID | 其余领域复用模板 |
...
| `API-001` | 自动 | 新 API 必须位于 v1、声明 Pydantic/OpenAPI 操作；报价请求/响应/参数/头已有合同测试 | 领域 Schema 持续登记 |
...
| `API-007` | 自动 | 旧 API 精确到函数和路径白名单；检查器允许适配器迁移但禁止端点数量增长；报价新旧合同测试 | 消费者迁移后删除兼容端点 |
