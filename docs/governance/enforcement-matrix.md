...
| --- | --- | --- | --- |
| `ARCH-001` | 部分自动 | 报价、产品和询价已有 Domain/Service/Repository/API 边界；行为测试证明 Web 与 AI 共用服务 | 合同、材料、发货和管理迁移 |
| `ARCH-002` | 部分自动 | 模块层依赖检查阻断 Domain/Service/Adapter 反向依赖；Inquiry 仅经 ProductService 公开目录端口跨域 | 增加跨领域公开服务依赖图 |
| `ARCH-003` | 自动 | 新路由数据库导入失败；报价、首页、产品、询价和旧内部 API 适配器已清零；旧 SQL 计数只减不增 | 逐域把计数降为零 |
...
| `SEC-003` | 部分自动 | 报价、产品和询价 Service 负责事务与审计；平台幂等记录 request ID；artifact 绑定服务端 Principal | 其余领域复用模板 |
