...
本机 5055 统一通过 `bash tools/restart_local_5055.sh` 或安装到“应用程序”文件夹的 `BLD` 启动器启动；两者复用同一套逻辑。它只负责通过 Terminal 启动当前工作区里的 5055 服务并打开浏览器，不用于 NAS。再次启动会先停止同一工作区的旧 5055 服务、精确关闭启动器自己创建且已停止的旧 Terminal 窗口，再启动新服务；其他目录占用 5055 时会提示且不会停止它。启动日志写入 `logs/bld-local-5055.log`。
