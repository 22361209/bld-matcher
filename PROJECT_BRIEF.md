...
- 具有 `sync_product_data` 权限的账号可使用“业务数据同步”，产品目录可选携带 `data/drawings/` 与 `data/product_images/`，并在导入确认时选择是否写入媒体。
- 产品导入会先预览新增、更新、冲突、无变化和本机独有数量；可明确选择停用包内不存在的本机产品。媒体文件原子写入，数据库导入失败时恢复本次媒体变更；旧产品专用数据包不再支持导入。
- 管理员菜单另有“业务数据同步”：同一个 `.tar.gz` 数据包可选择客户、产品目录、报价记录、管件资料和材料明细。导入必须先预览；产品和管件按编号、客户/报价/材料按跨设备同步标识增量合并；报价包含本机未登记客户时，预览要求逐个选择新建或映射到已有客户后才可导入。材料标识由业务字段生成，不依赖 Excel 文件名或行号；历史记录首次同步可接管统一标识。较旧数据会跳过，报价存在差异时始终标为冲突并保留当前记录；重复编号包，或预览后本机数据/数据包发生变化时均会阻断导入。每次导入前生成 SQLite 一致性备份并占用全局导入锁。该包不包含账号、权限、审计日志、产品图片/图纸或报价附件；客户负责人账号是设备本地身份，导入时不会覆盖。
