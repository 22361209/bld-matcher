...
- 物料图纸页读取 `data/material_drawings/*.pdf`，产品编码取 PDF 文件名 stem，当前类别默认“球销”；支持按编码搜索、类别筛选、编码自然排序，页面采用左侧编码列表、右侧固定 PDF 预览的工作台布局。物料图纸可随材料明细显式导出，导入预览后另行确认；同名 PDF 原子覆盖，包内没有的本机图纸保留。
