...
- 具备 `manage_customer_prices` 权限的用户可在查询结果中为唯一命中行指定启用产品或填写本次含税报价；没有目录价的产品也可报价，临时调整按工作表和源行稳定绑定，并同时用于 Excel 与报价写入，不修改产品目录默认值。一个源行展开出的多个 BLD 片段候选保持只读，避免把调整误写到其他源行。
