...
- 管理员菜单另有“业务数据同步”：同一个 `.tar.gz` 数据包可选择客户、产品目录、报价记录、管件资料和材料明细。导入必须先预览；产品和管件按编号、客户/报价/材料按跨设备同步标识增量合并；报价包含本机未登记客户时，预览要求逐个选择新建或映射到已有客户后才可导入。材料标识由业务字段生成，不依赖 Excel 文件名或行号；历史记录首次同步可接管统一标识。较旧数据会跳过，报价存在差异时始终标为冲突并保留当前记录；重复编号包，或预览后本机数据/数据包发生变化时均会阻断导入。每次导入前生成 SQLite 一致性备份并占用全局导入锁。该包不包含账号、权限、审计日志、产品图片/图纸或报价附件。
- 产品编辑页可上传/替换单个 PDF 图纸和最多 5 张产品图片，也可删除产品；已保存的每张图片和图纸都有独立删除按钮，删除后文件移入归档目录保留；手工清空含税单价会写入空值，图片/图纸格式校验失败时不会先更新产品文字资料。
- 新增/编辑产品表单的品牌、产品名称、产品状态通过选项选择器填写：候选来自受控词表 `product_option_values`（迁移播种 + 产品写入时自动登记新值），品牌为多选 combobox（chips 在输入框内），名称/状态为筛选单选，三者均支持无匹配时新增；管理菜单的“产品候选值”页（仅管理员）可对词表做新增/改名/删除，不影响已存产品数据；目录导入模板与校验同步只读该词表（ADR 0014）。
...
- 报价的客户和 BLD 号必须已登记（客户见管理菜单“客户列表”，改名级联历史记录；BLD 号见产品目录，含已停用产品），手动、Excel 导入、询价写入和 API v1 统一硬校验；录入时客户和 BLD 号用字符匹配下拉选择（方向键+回车），有 `manage_customers` 权限可在下拉中快捷新增客户（ADR 0016）。
...
- 产品目录、材料明细、管件资料、报价记录和询价结果使用统一数据表框体与列头控件：搜索命令区和表格同属一张数据卡片，表头左对齐垂直居中，内容支持双向滚动，列间使用浅色细分隔线，底栏整合当前范围、筛选总数和分页；列拖拽、筛选和候选浮层复用共享组件，所有列宽可拖动调整并按当前登录用户保存在浏览器中，拖动中断时会自动恢复页面交互状态。
