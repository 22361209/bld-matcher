...
- 具有 `sync_product_data` 权限的账号可使用“业务数据同步”：选择产品目录时可显式携带 `data/drawings/` 与 `data/product_images/`，选择材料明细时可显式携带 `data/material_drawings/` 根目录 PDF；导入预览按包内实际文件计数，并在确认时分别选择是否写入。同步包不写产品的图片/图纸引用列；产品图片在引用为空时按 BLD 文件名兜底查找 `data/product_images/`（含 `-2` 至 `-5` 多图位），导入图片后即时生效。
...
- 管理员菜单另有“业务数据同步”：同一个 `.tar.gz` 数据包可选择客户、产品目录、报价记录、管件资料和材料明细。导入必须先预览；产品和管件按编号、客户/报价/材料按跨设备同步标识增量合并；报价包含本机未登记客户时，预览要求逐个选择新建或映射到已有客户后才可导入。材料标识由业务字段生成，不依赖 Excel 文件名或行号；历史记录首次同步可接管统一标识。较旧数据会跳过，报价存在差异时始终标为冲突并保留当前记录；重复编号包，或预览后本机数据/数据包发生变化时均会阻断导入。每次导入前生成 SQLite 一致性备份并占用全局导入锁。新导出包使用 v3，导入仍兼容 v1/v2；产品图纸、产品图片和物料图纸均为显式选择的可选媒体，同名文件原子覆盖且失败时恢复，包内没有的本机媒体保留；归档资源用量受限，大小写或 Unicode 形式等价的媒体名按同一跨平台目标处理。该包不包含账号、权限、审计日志、报价附件或客户资料/客户图纸；客户负责人账号是设备本地身份，导入时不会覆盖。
