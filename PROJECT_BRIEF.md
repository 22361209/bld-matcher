...
- 具有 `sync_product_data` 权限的账号可使用“业务数据同步”：选择产品目录时可显式携带 `data/drawings/` 与 `data/product_images/`，选择材料明细时可显式携带 `data/material_drawings/` 根目录 PDF；导入预览按包内实际文件计数，并在确认时分别选择是否写入。
...
- 新增/编辑产品表单的品牌、产品名称、产品状态通过选项选择器填写：候选来自受控词表 `product_option_values`（迁移播种 + 产品写入时自动登记新值），品牌为多选 combobox（chips 在输入框内），名称/状态为筛选单选，三者均支持无匹配时新增；产品目录工具栏的“基础信息”需要 `manage_product_options` 权限，可对词表做新增/改名/删除，不影响已存产品数据；目录导入模板与校验同步只读该词表（ADR 0014）。
