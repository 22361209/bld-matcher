...
class LocalLauncherTests(unittest.TestCase):
    def test_launcher_restarts_only_its_own_service_and_closes_its_managed_terminal(self):
        ...
        self.assertIn("tell application \"Terminal\" to set targetBusy to busy of selected tab of targetWindow", launcher)
        self.assertIn("repeat 20 times", launcher)
        self.assertIn("if not targetBusy then", launcher)
